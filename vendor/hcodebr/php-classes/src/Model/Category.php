...
class Category extends Model {
    ...
    public function getProductsPage($page = 1, $itemsPerPage = 8)                   //retorna itens para paginacao
    {
        $start = ($page - 1) * $itemsPerPage;                                       //item inicial
        $sql = new Sql();
        $results = $sql->select("
        ...
        $resultTotal = $sql->select("SELECT FOUND_ROWS() AS nrtotal;");             //total de itens
        return [
            'data'=>Product::checkList($results),                                   //dados dos produtos
            'total'=>(int)$resultTotal[0]["nrtotal"],
            'pages'=>ceil($resultTotal[0]["nrtotal"] / $itemsPerPage)         //quant paginas, ceil converte arredondando para cima
        ];
    }
