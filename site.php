...
$app->get("/checkout", function(){
    ...
    if(isset($_GET['zipcode'])){                                //cep enviado
        $address->loadFromCEP($_GET['zipcode']);                //obj carregado com endereco

        $cart->setdeszipcode($_GET['zipcode']);

        $cart->save();

        $cart->getCalculateTotal();                             //mudou o cep, atualiza o frete
    }

    //se nao estiver setado, coloca vazio
...
$app->post("/checkout", function(){
    ...
    $address = new Address();
    $_POST['deszipcode'] = $_POST['zipcode'];                                   //nomes diferentes
...
$app->get("/order/:idorder", function($idorder){
    User::verifyLogin(false);
    $order = new Order();
    $order->get((int)$idorder);                                             //carrega pelo id

    $page = new Page();
    $page->setTpl("payment", [                                              //template
        'order'=>$order->getValues()                                        //argumentos
    ]);
});
...
$app->get("/profile/orders/:idorder", function($idorder){
    ...
    $order = new Order();
    $order->get((int)$idorder);
    $cart = new Cart();                                         //carrinho do pedido, nao pega da sessao
...
$app->get("/profile/change-password", function(){                               //pagina
    User::verifyLogin(false);
    $page = new Page();
...
$app->post("/profile/change-password", function(){                          //dados do formulario
    User::verifyLogin(false);
    if (!isset($_POST['current_pass']) || $_POST['current_pass'] === '') {  //nao definido ou vazio
    ...
    $user = User::getFromSession();
    if (!password_verify($_POST['current_pass'], $user->getdespassword())) {    //digitada com a que esta no banco
